...
AS
  ...
  VAR NUMBER;--ALMACENA EL CONTEO DE LOS REGISTRO DEL OBJETIVO
...
BEGIN 
   ...
   --VERIFICAR QUE EL TC2_TIPO_TARIFA SIEMPRE SEA 1 
   
    SELECT COUNT(TC2_TIPO_TARIFA)
    INTO VAR
	FROM QA_TTC2_REGISTRO
	WHERE TC2_MES_REPORTE   = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
	AND   TC2_AGNIO_REPORTE = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
	AND   TC2_TIPO_TARIFA <> 1
	;
	...
	IF VAR > 0 THEN
		...
		,	TC2_AGNIO_REPORTE
		,	TC2_MES_REPORTE
		,   'Error de reporte, no se permite la tarifa para usuario del mercado no regulado' AS TC2_OBS
	    BULK COLLECT INTO V_QA_TTC2_REGISTRO
		FROM QA_TTC2_REGISTRO
		WHERE TC2_MES_REPORTE   = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
		AND   TC2_AGNIO_REPORTE = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
