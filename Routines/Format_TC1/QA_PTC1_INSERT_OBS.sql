...
BEGIN
	...
	IF (AJUSTADO = 0) THEN
		...
		SET TC1_CODTRANSF = (SELECT TT2_CODE_IUA FROM QA_TTT2_REGISTRO WHERE TT2_ESTADO = 'OPERACION' AND TT2_CODIGOELEMENTO = T.TC1_CODCONEX)
		   ,TC1_IUA = (SELECT TT2_CODE_IUA FROM QA_TTT2_REGISTRO WHERE TT2_ESTADO = 'OPERACION' AND TT2_CODIGOELEMENTO = T.TC1_CODCONEX)
		   ,TC1_IDMERC = '161'
		   ,TC1_TIPCONEX = 'T'
	...
	IF (AJUSTADO = 1) THEN
		...
		UPDATE QA_TTC1_TEMP T
		SET TC1_CODTRANSF = (SELECT DISTINCT TC1_IUA FROM QA_TTC1_TEMP WHERE TC1_CODCONEX = T.TC1_CODCONEX)
		   ,TC1_IUA = (SELECT DISTINCT TC1_IUA FROM QA_TTC1_TEMP WHERE TC1_CODCONEX = T.TC1_CODCONEX)	
		   ,TC1_IDMERC = '161'
		   ,TC1_TIPCONEX = 'T'
		   ,TC1_LONGITUD = RPAD(TC1_LONGITUD,INSTR(TC1_LONGITUD,'.') + 6, 0)
           ,TC1_LATITUD  = RPAD(TC1_LATITUD, INSTR(TC1_LATITUD, '.') + 6, 0)
