CREATE OR REPLACE PROCEDURE BRAE.QA_PTC1_INSERT_OBS(FECHAOPERACION IN  NUMBER, USR IN VARCHAR2, ESTADO OUT NUMBER)
AS

QUANTITY_1 NUMBER;
QUANTITY_2 NUMBER;
ID_COMER NUMBER;
AJUSTADO NUMBER;
VAR_VALID VARCHAR2(100); 
MESSAGE VARCHAR2(100);
VAR_CODIGOTRF VARCHAR2(20);

BEGIN
	--Eliminar duplicidad en tabla OBS
	DELETE FROM QA_TTC1_OBS 
	WHERE ROWID IN (
					SELECT ROW_ID FROM(
					    SELECT ROWID AS row_id, ROW_NUMBER() OVER (PARTITION BY TC1_TC1,TC1_IDCOMER ORDER BY ROWID) AS RN
					    FROM QA_TTC1_OBS
					    WHERE TC1_PERIODO = FECHAOPERACION
						)
					WHERE RN > 1	
				   )
	;
	COMMIT;

	--Identificar el transformador agregado
	SELECT TC1_CODCONEX
	INTO VAR_CODIGOTRF
	FROM QA_TTC1_TEMP qtt 
	WHERE TC1_PERIODO = 0;

	--Verificar TC1_CODCONEX
	SELECT COUNT(DISTINCT FDD_AJUSTADO) - 1 AS QUANTITY 
	INTO AJUSTADO
	FROM   QA_TFDDREGISTRO
	WHERE  TO_CHAR(FDD_FINICIAL,'YYYYMM') = FECHAOPERACION; 
	
	IF (AJUSTADO = 0 OR VAR_CODIGOTRF = 'XT00001') THEN
	
		UPDATE QA_TTC1_TEMP T
		SET TC1_CODTRANSF = NVL((SELECT TT2_CODE_IUA 
								 FROM QA_TTT2_REGISTRO 
								 WHERE TT2_ESTADO = 'OPERACION' 
								 AND TT2_CODIGOELEMENTO = T.TC1_CODCONEX),'NA')
		    ,TC1_CODCIRC  = NVL((SELECT NVL(TT2_IUL,'NA') 
								 FROM QA_TTT2_REGISTRO 
								 WHERE TT2_ESTADO = 'OPERACION' 
								 AND TT2_CODIGOELEMENTO = T.TC1_CODCONEX),'NA')
		    ,TC1_IUA      = NVL((SELECT TT2_CODE_IUA 
		   						 FROM QA_TTT2_REGISTRO 
		   						 WHERE TT2_ESTADO = 'OPERACION' 
		   						 AND TT2_CODIGOELEMENTO = T.TC1_CODCONEX),'NA')
		    ,TC1_IDMERC    = '161'
		    ,TC1_TIPCONEX  = 'T'
		    ,TC1_LONGITUD  = RPAD(TC1_LONGITUD,INSTR(TC1_LONGITUD,'.') + 6, 0)
            ,TC1_LATITUD   = RPAD(TC1_LATITUD, INSTR(TC1_LATITUD, '.') + 6, 0)
            ,TC1_GC        = NVL((SELECT TT2_GRUPOCALIDAD 
            				      FROM QA_TTT2_REGISTRO 
            				      WHERE TT2_CODIGOELEMENTO = TC1_CODCONEX
            				      AND TT2_ESTADO = 'OPERACION'),'NA')
		   WHERE TC1_PERIODO = 0;
		COMMIT;
	ELSE
		IF (AJUSTADO = 1) THEN
		
			UPDATE QA_TTC1_TEMP T
			SET TC1_CODTRANSF = NVL((SELECT DISTINCT TC1_IUA 
									 FROM QA_TTC1_TEMP 
									 WHERE TC1_CODCONEX = T.TC1_CODCONEX 
									 AND TC1_PERIODO != 0),'NA')
			    ,TC1_CODCIRC  = NVL((SELECT NVL(TT2_IUL,'NA') 
									 FROM QA_TTT2_REGISTRO 
									 WHERE TT2_ESTADO = 'OPERACION' 
									 AND TT2_CODIGOELEMENTO = T.TC1_CODCONEX),'NA')
			    ,TC1_IUA      = NVL((SELECT DISTINCT TC1_IUA 
									 FROM QA_TTC1_TEMP 
									 WHERE TC1_CODCONEX = T.TC1_CODCONEX 
									 AND TC1_PERIODO != 0),'NA')	
			    ,TC1_IDMERC    = '161'
			    ,TC1_TIPCONEX  = 'T'
			    ,TC1_LONGITUD  = RPAD(TC1_LONGITUD,INSTR(TC1_LONGITUD,'.') + 6, 0)
	            ,TC1_LATITUD   = RPAD(TC1_LATITUD, INSTR(TC1_LATITUD, '.') + 6, 0)
	            ,TC1_GC        = NVL((SELECT TT2_GRUPOCALIDAD 
	            				      FROM QA_TTT2_REGISTRO 
	            				      WHERE TT2_CODIGOELEMENTO = TC1_CODCONEX
	            				      AND TT2_ESTADO = 'OPERACION'),'NA')
	            WHERE TC1_PERIODO = 0;
			COMMIT;
		
		END IF;	
	END IF;
	
	SELECT DISTINCT TC1_CODTRANSF
	INTO VAR_VALID
	FROM QA_TTC1_TEMP 
	WHERE TC1_PERIODO = 0;

	IF VAR_VALID = 'NA' THEN
		MESSAGE := 'El codigo del transformador no es valido';
		DBMS_OUTPUT.PUT_LINE('El codigo del transformador no es valido');
	
		DELETE FROM QA_TTC1_TEMP WHERE TC1_PERIODO = 0;
		COMMIT;
	
		INSERT INTO QA_TLOG_EJECUCION
    	SELECT SYSDATE,'QA_PTC1_INSERT_OBS','PROCEDIMIENTO',UPPER(sys_context('USERENV','OS_USER')),'FALLIDO','Codigo erroneo del transformador' FROM DUAL;
   		COMMIT;
		
   		ESTADO := 0;
		RETURN;
	END IF;

    --CORREGIR CARACTERES EN EL CAMPO DIRRECION
    UPDATE QA_TTC1_TEMP
    SET TC1_DIREC = SUBSTR(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TC1_DIREC,'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U'),'á','a'),'é','e'),'í','i'),'ó','o'),'ú','u'),'Ñ','N'),'(',' '),')',' '),'\',' '),'/',' '),',',' '),'.',' '),'°',' '),'À',' '),'1º',' '),'2°',' '),'3°',' '),'º',' '),'·',' '),'ñ','n'),'´',' '),' ',' '),'#',' '),'ª',' '),'Ò',' '),'¿',''),'Â¿',''),'Â',''),1,50)
    WHERE TC1_PERIODO = 0;
    COMMIT;
   
    --COMPLETAR EL CAMPO CODIGO DE FRONTERA PARA LOS USUARIOS INCUMBENTES CON "OR0001"
    UPDATE QA_TTC1_TEMP
    SET   TC1_CODFRONCOM = 'OR0001'
    WHERE TC1_CODFRONCOM IS NULL
    AND   TC1_IDCOMER = '604'
    AND   TC1_PERIODO = 0
    ;
    COMMIT;

	--Verficar 	TC1_CODDANE
   	SELECT NVL(TC1_MUNICIPIO,'NA')
   	INTO VAR_VALID
   	FROM QA_TTC1_TEMP
   	LEFT OUTER JOIN QA_TTC1_DIVIPOLA ON TC1_COD_CENTRO_POBLADO = TC1_CODDANE
   	WHERE TC1_PERIODO = 0;
   
   	IF VAR_VALID = 'NA' THEN 
   		
		MESSAGE := 'El codigo DANE no es valido';
		DBMS_OUTPUT.PUT_LINE('El codigo DANE no es valido');
	
		DELETE FROM QA_TTC1_TEMP WHERE TC1_PERIODO = 0;
		COMMIT;
	
		INSERT INTO QA_TLOG_EJECUCION
    	SELECT SYSDATE,'QA_PTC1_INSERT_OBS','PROCEDIMIENTO',UPPER(sys_context('USERENV','OS_USER')),'FALLIDO','Error en el codigo DANE' FROM DUAL;
   		COMMIT;
   	
   		ESTADO := 0;
		RETURN;
	
   	END IF;
	--Asignar 	TC1_PERIODO
   
   	UPDATE QA_TTC1_TEMP 
   	SET TC1_PERIODO = FECHAOPERACION
   	WHERE TC1_PERIODO = 0;
   	COMMIT;

	--Eliminar el usuario de la tabla observaciones, luego de verificar que todo ha sido exitoso
	DELETE FROM QA_TTC1_OBS 
	WHERE TC1_PERIODO = FECHAOPERACION
	AND   TC1_TC1 = USR; 
	COMMIT;

	DBMS_OUTPUT.PUT_LINE('El registro se ha insertado exitosamente');
	MESSAGE := 'El registro se ha insertado exitosamente';
	ESTADO := 1;

	INSERT INTO QA_TLOG_EJECUCION
	SELECT SYSDATE,'QA_PTC1_INSERT_OBS','PROCEDIMIENTO',UPPER(sys_context('USERENV','OS_USER')),'EXITOSO','Procedimiento ejecutado sin errores' FROM DUAL;
	COMMIT;

END QA_PTC1_INSERT_OBS;