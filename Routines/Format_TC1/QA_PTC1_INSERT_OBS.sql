...
BEGIN
	...
	IF (AJUSTADO = 0) THEN
		...
		SET TC1_CODTRANSF = NVL((SELECT TT2_CODE_IUA FROM QA_TTT2_REGISTRO WHERE TT2_ESTADO = 'OPERACION' AND TT2_CODIGOELEMENTO = T.TC1_CODCONEX),'NA')
		   ,TC1_IUA       = NLV((SELECT TT2_CODE_IUA FROM QA_TTT2_REGISTRO WHERE TT2_ESTADO = 'OPERACION' AND TT2_CODIGOELEMENTO = T.TC1_CODCONEX),'NA')
		   ,TC1_IDMERC    = '161'
		   ,TC1_TIPCONEX  = 'T'
	...
	IF (SELECT DISTINCT TC1_CODTRANSF FROM QA_TTC1_TEMP WHERE TC1_PERIODO IS NULL) = 'NA' THEN
