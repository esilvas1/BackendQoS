...
BEGIN
	...
	DELETE FROM QA_TDIFERENCIA_USUARIOS 
	WHERE  FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
	AND    FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
	;
	COMMIT;
	...
	--USURIOS CON DIFERENCIA EN LA ASIGNANCION DEL CODIGO DE CONEXION IUA/IUL
	INSERT INTO QA_TDIFERENCIA_USUARIOS
	SELECT LAC.FDD_YEAR
		  ,LAC.FDD_MES
		  ,LAC.FDD_NIU
		  ...
		  ,LAC.FDD_TIEMPONETO 
		  ,LAC.FDD_ES_ALUMBRADO 
		  ,'Usuarios con diferencia en el codigo de conexión IUA/IUL' AS FDD_OBSERVACIONES
	FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC
	WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
	AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
	AND   LAC.FDD_NIU IN (
						  SELECT LAC.FDD_NIU
						  FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC 
						  LEFT OUTER JOIN (SELECT FDD_NIU, FDD_CODIGOELEMENTO 
						  				   FROM QA_TCONSOLIDADO_OR_USUARIOS
						  				   WHERE FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  				   AND   FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
						  				   ) ORR ON ORR.FDD_NIU = LAC.FDD_NIU
						  WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
						  AND   LAC.FDD_CODIGOELEMENTO <> ORR.FDD_CODIGOELEMENTO
						 )
	;
	COMMIT;

	--USUARIOS CON DIFERENCIAS EN LA ASIGNACION DEL GRUPO DE CALIDAD
	INSERT INTO  QA_TDIFERENCIA_USUARIOS
	SELECT LAC.FDD_YEAR
		  ,LAC.FDD_MES
		  ,LAC.FDD_NIU
		  ...
		  ,LAC.FDD_TIEMPONETO 
		  ,LAC.FDD_ES_ALUMBRADO 
		  ,'Usuarios con diferencia la asignación del GRUPO DE CALIDAD' AS FDD_OBSERVACIONES
	FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC
	WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
	AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'))
	AND   LAC.FDD_NIU IN (
						  SELECT LAC.FDD_NIU
						  FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC 
						  LEFT OUTER JOIN (SELECT FDD_NIU, FDD_GRUPOCALIDAD  
						  				   FROM QA_TCONSOLIDADO_OR_USUARIOS
						  				   WHERE FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  				   AND   FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'))
						  				   ) ORR ON ORR.FDD_NIU = LAC.FDD_NIU
						  WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'))
						  AND   LAC.FDD_GRUPOCALIDAD  <> ORR.FDD_GRUPOCALIDAD
						 );
	COMMIT;

	--USUARIOS CON DIFERENCIAS EN EL NIVEL DE TENSION
	INSERT INTO QA_TDIFERENCIA_USUARIOS
	SELECT LAC.FDD_YEAR
		  ,LAC.FDD_MES
		  ,LAC.FDD_NIU
		  ...
		  ,LAC.FDD_TIEMPONETO 
		  ,LAC.FDD_ES_ALUMBRADO 
		  ,'Usuarios con diferencia la asignación del NIVEL DE TENSION' AS FDD_OBSERVACIONES
	FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC
	WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
	AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
	AND   LAC.FDD_NIU IN (
						  SELECT LAC.FDD_NIU
						  FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC 
						  LEFT OUTER JOIN (SELECT FDD_NIU, FDD_NIVEL  
						  				   FROM QA_TCONSOLIDADO_OR_USUARIOS
						  				   WHERE FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  				   AND   FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
						  				   ) ORR ON ORR.FDD_NIU = LAC.FDD_NIU
						  WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
						  AND   LAC.FDD_NIVEL  <> ORR.FDD_NIVEL
						 );
	COMMIT;

	--USUARIOS CON DIFERENCIA EN LA ASIGNACION DE ALUMBRADO PUBLICO
	INSERT INTO QA_TDIFERENCIA_USUARIOS
	SELECT LAC.FDD_YEAR
		  ,LAC.FDD_MES
		  ,LAC.FDD_NIU
		  ...
		  ,LAC.FDD_TIEMPONETO 
		  ,LAC.FDD_ES_ALUMBRADO 
		  ,'Usuarios con diferencia la asignación del ALUMBRADO PUBLICO' AS FDD_OBSERVACIONES
	FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC
	WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
	AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
	AND   LAC.FDD_NIU IN (
						  SELECT LAC.FDD_NIU
						  FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC 
						  LEFT OUTER JOIN (SELECT FDD_NIU, FDD_ES_ALUMBRADO  
						  				   FROM QA_TCONSOLIDADO_OR_USUARIOS
						  				   WHERE FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  				   AND   FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
						  				   ) ORR ON ORR.FDD_NIU = LAC.FDD_NIU
						  WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
						  AND   LAC.FDD_ES_ALUMBRADO <> ORR.FDD_ES_ALUMBRADO
						 );
	COMMIT;

	--USUARIOS CON DIFERENCIA EN LA DURACION
	INSERT INTO QA_TDIFERENCIA_USUARIOS
	SELECT LAC.FDD_YEAR
		  ,LAC.FDD_MES
		  ,LAC.FDD_NIU
		  ...
		  ,LAC.FDD_TIEMPONETO 
		  ,LAC.FDD_ES_ALUMBRADO 
		  ,'Usuarios con diferencia en la DURACIÓN' AS FDD_OBSERVACIONES
	FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC
	WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
	AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
	AND   LAC.FDD_NIU IN (
						  SELECT LAC.FDD_NIU
						  FROM QA_TCONSOLIDADO_LAC_USUARIOS LAC 
						  LEFT OUTER JOIN (SELECT FDD_NIU, FDD_TIEMPONETO  
						  				   FROM QA_TCONSOLIDADO_OR_USUARIOS
						  				   WHERE FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  				   AND   FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
						  				   ) ORR ON ORR.FDD_NIU = LAC.FDD_NIU
						  WHERE LAC.FDD_YEAR = TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))
						  AND   LAC.FDD_MES  = TO_NUMBER(TO_CHAR(FECHAOPERACION,'MM'  ))
						  AND   ROUND(LAC.FDD_TIEMPONETO,10) <> ROUND(ORR.FDD_TIEMPONETO, 10)
						  AND   ABS(ROUND(LAC.FDD_TIEMPONETO,10) - ROUND(ORR.FDD_TIEMPONETO, 10)) > 0.001
						 );
	COMMIT;
