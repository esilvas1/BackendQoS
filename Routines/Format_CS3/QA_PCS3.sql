CREATE OR REPLACE PROCEDURE BRAE.QA_PCS3(FECHAOPERACION DATE)
AS
    ...
    --Para determinar el valor del incentivo fijo, If_SAIDIj,t, se debe utilizar el indicador de calidad SAIDIj,t-1 y tener en cuenta lo siguiente:
         ...
         --c. Si el SAIDIj,t-1 es mayor que el límite superior de la banda de indiferencia de su meta para el año t-1, el If_SAIDIj,t se obtiene utilizando la siguiente expresión:
                      --If_SAIDI_(j,t)=-0,04*máx(0,04*∑_(n=1)^3 Crr_(j,n)   ,∑_(n=1)^3 BRAEN_(j,n,t-1) )

BEGIN
         --PERIODO DE OPERACION
         INSERT INTO BRAE.QA_TCS3 (CS3_PERIODO_OP) VALUES (TRUNC(FECHAOPERACION));
         ...
         --Ivi_SAIDImax_(j,t)=0,04*∑_(n=1)^3 BRAEN_(j,n,t-1)
         ...
         --Ivi_SAIFImax_(j,t)=0,04*∑_(n=1)^3 BRAEN_(j,n,t-1) 
          SELECT  0.04*(CB_BRAENJ1+CB_BRAENJ2+CB_BRAENJ3) 
          INTO V_IV_SAIFI_MAX
          FROM QA_TCBRAEN 
          WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1); 
         ...
         --If_SAIDI_(j,t)=0,04*∑_(n=1)^3 BRAEN_(j,n,t-1) 
         --CALCULAR INCENTIVO FIJO POR DURACION Y FRECUENCIA
          UPDATE BRAE.QA_TCS3
          SET CS3_IF_SAIDI = 0.04*(SELECT  (CB_BRAENJ1+CB_BRAENJ2+CB_BRAENJ3) FROM QA_TCBRAEN WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1)),
              CS3_IF_SAIFI = 0.04*(SELECT  (CB_BRAENJ1+CB_BRAENJ2+CB_BRAENJ3) FROM QA_TCBRAEN WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1))
         ...
         --- ACTUALIZAR CAMPOS AGRUPADOS Y PRORRATEADOS DEL CS2
          UPDATE BRAE.QA_TCS3
          SET CS3_IC_SAIDI = (SELECT CS3_IF_SAIDI + CS3_IV_SAIDI FROM QA_TCS3 WHERE CS3_PERIODO_OP=TRUNC(FECHAOPERACION)),
              CS3_IC_SAIFI = (SELECT CS3_IF_SAIFI + CS3_IV_SAIFI FROM QA_TCS3 WHERE CS3_PERIODO_OP=TRUNC(FECHAOPERACION))
          WHERE CS3_PERIODO_OP=TRUNC(FECHAOPERACION);

        UPDATE BRAE.QA_TCS3
          SET CS3_INCD1= CS3_IC_SAIDI * (SELECT (CB_BRAEJ1/(CB_BRAEJ1+CB_BRAEJ2+CB_BRAEJ3)) FROM QA_TCBRAEN WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1)),
              CS3_INCD2= CS3_IC_SAIDI * (SELECT (CB_BRAEJ2/(CB_BRAEJ1+CB_BRAEJ2+CB_BRAEJ3)) FROM QA_TCBRAEN WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1)),
              CS3_INCD3= CS3_IC_SAIDI * (SELECT (CB_BRAEJ3/(CB_BRAEJ1+CB_BRAEJ2+CB_BRAEJ3)) FROM QA_TCBRAEN WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1)),
              CS3_INCF1= CS3_IC_SAIFI * (SELECT (CB_BRAEJ1/(CB_BRAEJ1+CB_BRAEJ2+CB_BRAEJ3)) FROM QA_TCBRAEN WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1)),
              CS3_INCF2= CS3_IC_SAIFI * (SELECT (CB_BRAEJ2/(CB_BRAEJ1+CB_BRAEJ2+CB_BRAEJ3)) FROM QA_TCBRAEN WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1)),
              CS3_INCF3= CS3_IC_SAIFI * (SELECT (CB_BRAEJ3/(CB_BRAEJ1+CB_BRAEJ2+CB_BRAEJ3)) FROM QA_TCBRAEN WHERE CB_PERIODO=(TO_NUMBER(TO_CHAR(FECHAOPERACION,'YYYY'))-1))
          WHERE CS3_PERIODO_OP=TRUNC(FECHAOPERACION);
        COMMIT;
...
BEGIN
DELETE FROM QA_TCS3;
QA_PCS3(TO_DATE('01/01/2020','DD/MM/YYYY'));
END;
