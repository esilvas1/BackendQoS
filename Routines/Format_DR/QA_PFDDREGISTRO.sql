...
IF CONTADOR = 0 AND CANTIDAD > 0 THEN --NO PERMITE REPORTE DE INFORMACION SIN QUE SE HAYA REPORTADO EL DIA ANTERIOR
  ...
  --REVISAR Y AJUSTAR SI EXISTEN EVENTOS NR QUE HAN CERRADO CON CIERRES TARDIOS Y SU DURACION ES NEGATIVA
    ...
    --ESCRITURA EN TABLA LOG
         ...
         SELECT SYSDATE,'QA_PFDDREGISTRO','PROCEDIMIENTO',UPPER(sys_context('USERENV','OS_USER')),'EXITOSO','Proceso ejecutado sin problemas' FROM DUAL;
          
  ELSE
    --ESCRITURA EN TABLA LOG
         INSERT INTO QA_TLOG_EJECUCION
         SELECT SYSDATE,'QA_PFDDREGISTRO','PROCEDIMIENTO',UPPER(sys_context('USERENV','OS_USER')),'FALLIDO','Intenta realizar un periodo ejecutado o un periodo anterior' FROM DUAL;
