...
SELECT ((SELECT COUNT(TC1_TC1) FROM QA_TTC1_TEMP      WHERE TC1_PERIODO=202401)
       -(SELECT COUNT(TC1_TC1) FROM QA_TTC1           WHERE TC1_PERIODO=202312)) AS CANTIDAD_DE_AUMENTO
FROM DUAL;--1701
...
,TC1_IUA AS "Codigo de Conexi�n"
,TC1_TIPCONEX AS "Tipo de Conexi�n"
,TC1_NT AS "Nivel de Tensi�n del usuario"
,TC1_NTP AS "Nivel de Tensi�n"
,(TC1_PROPACTIV) AS "% propiedad del Activo"
,(TC1_CONEXRED) AS "Conexi�n Red"
,(TC1_IDCOMER) AS "ID Comercializador"
,(TC1_IDMERC) AS "ID Mercado"
,(TC1_GC) AS "GRUPO CALIDAD"
,TC1_CODFRONCOM AS "C�digo Frontera Comercial"
,(TC1_CODCIRC) AS "C�digo Circuito o L�nea"
,TC1_CODTRANSF AS "CODIGO TRANFORMADOR TT2"
,(TC1_CODDANE) AS "C�digo DANE NIU"
,(TC1_UBIC) AS "Ubicaci�n"
...
,TC1_EXPENER AS "Exporta Energ�a"
,TC1_CAPAUTOGENR AS "Capacidad Autogenerador kw"
,TC1_TIPGENR AS "Tipo de Generaci�n"
,TC1_CODFRONEXP AS "Codigo frontera Autogenarador"
,TC1_FENTGEN AS "Fecha Entrada en Operaci�n"
...
SELECT DISTINCT TC1_CODCONEX FROM BRAE.QA_TTC1 WHERE TC1_PERIODO=202401
MINUS
...
MINUS
SELECT DISTINCT TC1_CODCONEX FROM QA_TTC1 WHERE TC1_PERIODO=202401;
...
AND I.FINICIAL >= TO_DATE('01/12/2023','DD/MM/YYYY')
AND I.FINICIAL  <  TO_DATE('01/01/2024','DD/MM/YYYY')
AND I.TRAFO='5T02735';
...
AND I.TRAFO IN (SELECT DISTINCT TC1_CODCONEX FROM QA_TTC1_TEMP
              MINUS
               SELECT DISTINCT TC1_CODCONEX FROM QA_TTC1 WHERE TC1_PERIODO=202312
               ); --VACIA
...
FROM (
        ...
        MINUS
        SELECT DISTINCT TC1_CODCONEX,TC1_IUA FROM QA_TTC1 WHERE TC1_PERIODO=202401
     )
MINUS
(SELECT DISTINCT TC1_CODCONEX FROM QA_TTC1_TEMP
MINUS
...
WHERE TC1_CODCONEX IN (
                        ...
                        FROM(
                        SELECT DISTINCT TC1_CODCONEX AS X,TC1_IUA
                        FROM QA_TTC1 WHERE TC1_PERIODO=202401) T
                        HAVING COUNT(T.X) > 1 
                        GROUP BY T.X
                      );
...
WHERE TC1_PERIODO=202401; --618.440  Filas

DELETE FROM QA_TTC1
WHERE TC1_PERIODO = 202401; -- 618,440 rows affected 
COMMIT;

--INSERTAR QA_TTC1_TEMP A QA_TTC1
INSERT INTO QA_TTC1
SELECT * FROM QA_TTC1_TEMP; --618.459 filas insertadas.

COMMIT;

--Verificacion de la cantidad insertada
SELECT COUNT(*) FROM QA_TTC1 WHERE TC1_PERIODO=202401; --618.459 filas insertadas
--OK
